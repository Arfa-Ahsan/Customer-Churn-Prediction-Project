...
# Initialize scaler if needed (make sure to use the scaler that was used during training)
...
def churn_prediction(input_data):
    ...
    df = pd.DataFrame([input_data]).reindex(columns=original_columns, fill_value=0)
    
    # Scale the input data if needed (comment this out if no scaling was applied during model training)
    df[['tenure', 'monthlycharges', 'totalcharges']] = scaler.fit_transform(df[['tenure', 'monthlycharges', 'totalcharges']])
    
    # Predict the churn and the probabilities
    prediction = model.predict(df)[0]
    probabilities = model.predict_proba(df)[0]  # Returns probabilities [No Churn, Churn]
...
def main():
    ...
    input_data = {
        ...
        'gender_Male': gender,  # Fixed: Assigns 0 or 1 based on selection
        ...
        'paperlessbilling_Yes': paperlessbilling,  # Correcting for binary value
    ...
    if st.button("Predict"):
        churn_prediction_result, churn_probabilities = churn_prediction(input_data)

        # Extract the individual probabilities
        no_churn_probability = churn_probabilities[0]  # Probability of No Churn
        churn_probability = churn_probabilities[1]     # Probability of Churn
        ...
        if churn_prediction_result == 1:
            ...
            st.markdown(f'<div class="result-div"><p style="color: #50de40;font-weight:bold; font-size: 20px;">The customer is unlikely to churn. The probability of no churn is: <span>{no_churn_probability:.2f}</span></p></div>', unsafe_allow_html=True)
        
        # Plot the probabilities using a bar chart
